...
install_path = if node.webserver.using_ree
                 node[:ruby_enterprise][:install_path]
               else
                 node[:ruby_mri][:install_path]
               end

execute "passenger_apache2_module" do
  command "#{install_path}/bin/passenger-install-apache2-module _#{node[:passenger_enterprise][:version]}_ -a"
